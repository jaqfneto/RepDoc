...
function formhash(form, password, email) {
   ...
   // form.submit();
    process_login(password.value,p.value,uEmail);
}
 
function process_login(pass,ePass,email) {
    alert("oi_process_login  "+email);
        ...
        xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
                ...
                document.getElementById("files").innerHTML = "process login";
                location.reload();
            }
        };
        var postData="email="+email+"&p="+ePass+"&password="+pass; 
